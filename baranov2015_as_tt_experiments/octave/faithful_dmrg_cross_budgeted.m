function [y, info]=faithful_dmrg_cross_budgeted(d,n,fun,eps,varargin)
...
%---------------------------
%Default parameters
info = struct('stopped_by_budget', false, 'message', '', 'sweeps_completed', 0, 'last_block', 0, 'converged', false, 'returned_checkpoint', false);
rmin=1;
verb=true;
radd=0;
kickrank=2;
...
completed_sweeps = 0;
y_last_good_block = y;
info_last_good_block = struct('stopped_by_budget', false, 'message', '', 'sweeps_completed', 0, 'last_block', 0, 'converged', false, 'returned_checkpoint', false);
y_last_good_sweep = [];
info_last_good_sweep = [];
...
%     big_index=reshape(big_index,[numel(big_index)/d,d]);
    ...
    try
        ...
        if strcmp(err.identifier, 'Faithful:BudgetExceeded')
            if ~isempty(y_last_good_sweep)
                y = y_last_good_sweep;
                info = info_last_good_sweep;
            else
                y = y_last_good_block;
                info = info_last_good_block;
            end
            info.stopped_by_budget = true;
            info.message = err.message;
            info.converged = false;
            info.returned_checkpoint = true;
            return
        end
        rethrow(err);
    end
...
%     size(v)
    ...
    if ( dir == 1 ) 
        ...
        y_last_good_block = y;
        info_last_good_block = struct('stopped_by_budget', false, 'message', '', 'sweeps_completed', completed_sweeps, 'last_block', i, 'converged', false, 'returned_checkpoint', false);
        if ( i == d - 1 ) 
            dir = -dir;
        else
            i=i+1;
        ...
        if ( i == 1 ) 
            ...
            y_last_good_sweep = y;
            info_last_good_sweep = struct('stopped_by_budget', false, 'message', '', 'sweeps_completed', completed_sweeps, 'last_block', completed_block, 'converged', false, 'returned_checkpoint', false);
        else
            i=i-1;
        end
        y_last_good_block = y;
        info_last_good_block = struct('stopped_by_budget', false, 'message', '', 'sweeps_completed', completed_sweeps, 'last_block', completed_block, 'converged', false, 'returned_checkpoint', false);
    end
end
if ~isempty(y_last_good_sweep)
    info = info_last_good_sweep;
...
info.stopped_by_budget = false;
info.message = '';
info.sweeps_completed = completed_sweeps;
info.converged = ~not_converged;
info.returned_checkpoint = false;
return
end
